...
# Edited: T. Mallen-Ntiador 07/09/26
...
def _sparse_global_avg_pool(
    ...
    if B is None:
        if coords.numel() == 0:
            raise ValueError("Cannot infer batch size from an empty SparseTensor")
    ...
    if coords.numel() > 0:
        batch = coords[:, 0].long()    # sparse_collate: (batch, x, y, z)
...
class SparseSimCLR(nn.Module):
    ...
    def encode(self, x: SparseTensor) -> torch.Tensor:
        ...
        batch_size = getattr(x, "batch_size", None)
        feature_maps = self.backbone(x)            # list of SparseTensors
        return _sparse_global_avg_pool(feature_maps[-1], batch_size=batch_size)
