...
def build_kmap_implicit_GEMM_hashmap(
    ...
    # spdownsample can produce 0 output voxels for small/sparse inputs.
    # lookup_coords would then launch a CUDA kernel with gridDim.x=0,
    # which is illegal (cudaErrorInvalidConfiguration).
    if coords.shape[0] == 0:
        kmap["out_in_map"] = torch.full(
            (0, int(torch.prod(kernel_size).item())),
            -1, dtype=torch.int32, device=_coords.device,
        )
        kmap["coords"] = coords
