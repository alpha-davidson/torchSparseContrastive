...
def conv3d(
    ...
    # Guard: empty SparseTensor input — skip all CUDA work, return empty output.
    # spdownsample and lookup_coords launch CUDA kernels sized by voxel count;
    # launching with 0 voxels produces gridDim.x=0 (cudaErrorInvalidConfiguration).
    if feats.shape[0] == 0:
        out_feats = feats.new_zeros((0, weight.shape[-1]))
        if bias is not None:
            out_feats = out_feats + bias
        out_stride = tuple(input.stride[k] * stride[k] for k in range(3))
        output = SparseTensor(coords=coords, feats=out_feats, stride=out_stride,
    ...
    if kernel_size == (1, 1, 1) and stride == (1, 1, 1) and dilation == (1, 1, 1):
        ...
        # Guard: spdownsample can reduce all voxels away for thin point clouds.
        # ConvolutionFunction.apply with 0 output voxels launches gridDim.x=0.
        if kmap["coords"].shape[0] == 0:
            out_feats = feats.new_zeros((0, weight.shape[-1]))
            if bias is not None:
                out_feats = out_feats + bias
            out_stride = tuple(input.stride[k] * stride[k] for k in range(3))
            output = SparseTensor(coords=kmap["coords"], feats=out_feats,
                                  stride=out_stride, spatial_range=kmap["spatial_range"])
            output._caches = input._caches
            output._caches.cmaps.setdefault(output.stride, (output.coords, output.spatial_range))
            return output
